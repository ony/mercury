...
:- func robdd_to_xrobdd(robdd(T)) = tfeirn(T).
...
one = xrobdd(init, init, init_equiv_vars, init_imp_vars, one, yes).

zero = xrobdd(init, init, init_equiv_vars, init_imp_vars, zero, yes).

xrobdd(TA, FA, EA, IA, RA, _) * xrobdd(TB, FB, EB, IB, RB, _) = 
		xrobdd(TA1 `union` TB1, FA1 `union` FB1,
			EA1 * EB1, IA1 * IB1, RA1 * RB1, no) :-
	TU = TA `union` TB,
	FU = FA `union` FB,
	EU = EA * EB,
	IU = IA * IB,
	xrobdd(TA1, FA1, EA1, IA1, RA1, _) =
		normalise(xrobdd(TU, FU, EU, IU, RA, no)),
	xrobdd(TB1, FB1, EB1, IB1, RB1, _) =
		normalise(xrobdd(TU, FU, EU, IU, RB, no)).

xrobdd(TA0, FA0, EA0, IA0, RA0, NA0) + xrobdd(TB0, FB0, EB0, IB0, RB0, NB0) =
		X :-
	( RA0 = zero ->
	    X = xrobdd(TB0, FB0, EB0, IB0, RB0, NB0)
	; RB0 = zero ->
	    X = xrobdd(TA0, FA0, EA0, IA0, RA0, NA0)
	;
	    X = xrobdd(T, F, E, I, R, no),
	    T = TA0 `intersect` TB0,
	    F = FA0 `intersect` FB0,
	    E = EA + EB,
	    I = IA + IB,
...
restrict(V, xrobdd(T, F, E, I, R, N)) =
	( T `contains` V ->
	    xrobdd(T `delete` V, F, E, I, R, N)
	; F `contains` V ->
	    xrobdd(T, F `delete` V, E, I, R, N)
	; L = E ^ leader(V) ->
	    ( L \= V ->
		xrobdd(T, F, E `delete` V, I, R, N)
	    ;
		xrobdd(T, F, E `delete` V, I `delete` V, restrict(V, R), N)
	    )
	;
	    xrobdd(T, F, E, I `delete` V, restrict(V, R), N)
	).

restrict_threshold(V, xrobdd(T, F, E, I, R, N)) =
...
eq_vars(VarA, VarB, X) = 
	...
	->
		zero
	;
		xrobdd(T, F, add_equality(VarA, VarB, E), I, R, no)
	) :-
	X = xrobdd(T, F, E, I, R, _).
...
neq_vars(VarA, VarB, X) = 
	...
	->
		X
	;
		xrobdd(T, F, E, I ^ neq_vars(VarA, VarB), R, no)
	) :-
	X = xrobdd(T, F, E, I, R, _).
...
imp_vars(VarA, VarB, X) =
	...
	; F `contains` VarA ->
		X
	;
		xrobdd(T, F, E, I ^ imp_vars(VarA, VarB), R, no)
	) :-
	X = xrobdd(T, F, E, I, R, _).
...
conj_vars(Vars, X) =
	...
	; \+ empty(Vars `intersect` F) ->
		zero
	;
		xrobdd(T `union` Vars, F, E, I, R, no)
	) :-
	X = xrobdd(T, F, E, I, R, _).
...
conj_not_vars(Vars, X) =
	...
	; \+ empty(Vars `intersect` T) ->
		zero
	;
		xrobdd(T, F `union` Vars, E, I, R, no)
	) :-
	X = xrobdd(T, F, E, I, R, _).
...
at_most_one_of(Vars, X) =
	...
	; count(Vars `intersect` T) > 1 ->
		zero
	;
		xrobdd(T, F, E, I ^ at_most_one_of(Vars), R, no)
	) :-
	X = xrobdd(T, F, E, I, R, _).
...
not_both(VarA, VarB, X) =
	...
	; T `contains` VarB ->
		not_var(VarA, X)
	;
		xrobdd(T, F, E, I ^ not_both(VarA, VarB), R, no)
	) :-
	X = xrobdd(T, F, E, I, R, _).
...
var_restrict_true(V, xrobdd(T, F, E, I, R, N)) = X :-
	( F `contains` V ->
	    X = zero
	; T `contains` V ->
	    X = xrobdd(T `delete` V, F, E, I, R, N)
	;
	    X0 = normalise(xrobdd(T `insert` V, F, E, I, R, no)),
	    X = X0 ^ true_vars := X0 ^ true_vars `delete` V
	).

var_restrict_false(V, xrobdd(T, F, E, I, R, N)) = X :-
	( T `contains` V ->
	    X = zero
	; F `contains` V ->
	    X = xrobdd(T, F `delete` V, E, I, R, N)
	;
	    X0 = normalise(xrobdd(T, F `insert` V, E, I, R, no)),
	    X = X0 ^ false_vars := X0 ^ false_vars `delete` V
	).

restrict_filter(P, xrobdd(T, F, E, I, R, N)) =
	xrobdd(filter(P, T), filter(P, F), filter(P, E), filter(P, I),
		restrict_filter(P, R), N).

labelling(Vars0, xrobdd(T, F, E, I, R, N), TrueVars, FalseVars) :-
	...
	;
	    labelling_2(Vars, xrobdd(init, init, E, I, R, N), TrueVars1,
	    	FalseVars1),
...
normalise(X) = X :-
	X ^ normalised = yes.
normalise(xrobdd(TrueVars0, FalseVars0, EQVars0, ImpVars0, Robdd0, no)) = X :-
	% T <-> F
...
robdd_to_xrobdd(R) =
	normalise(xrobdd(init, init, init_equiv_vars, init_imp_vars, R, no)).
...
to_robdd(X) = 
	...
	^ expand_implications(X ^ imp_vars).
%*/
